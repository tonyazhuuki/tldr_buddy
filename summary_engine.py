...
class SummaryEngine:
    ...
    def _get_chat_system_prompt(self) -> str:
        ...
        return f"""Сожми текст/транскрипт. Коротко, по порядку, без воды.

Формат:
▶ КОНСПЕКТ (до 5 строк, строго по порядку): • …
⚡ ДЕЙСТВИЕ: одна строка ИЛИ «нет»
🎭 ТОН: 1–2 слова

Запреты:
— Не повторяй факты в разных блоках.
— Не пиши общие формулы («обсуждение вида/планов»).
— «Писать/подумать/обсудить» без результата — не считать действием.

Параметры: model: gpt-5-mini, temperature: 0.2–0.25, frequency_penalty: 0.35, reasoning: {{ effort: "minimal" }}, max_tokens: 500–700."""

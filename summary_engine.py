...
class SummaryEngine:
    ...
    def _get_chat_system_prompt(self) -> str:
        ...
        return f"""Сожми текст/транскрипт. Коротко, по порядку, без воды. Верни СТРОГО в формате ниже и ничего лишнего.

🧭 КОНСПЕКТ (до 5 строк, по порядку):
• ...
• ...
• ...
• ...
...
⚡ ДЕЙСТВИЕ: одна строка ИЛИ «нет»

🎭 ТОН: 2–3 слова

Запреты:
— Не повторяй факты в разных блоках.
— Не пиши общие формулы типа «обсуждение вида/планов».
— «Писать/подумать/обсудить» без результата — не считать действием.

Правила верстки:
— Каждый пункт начинается с «• » и на своей строке.
— Между разделами — одна пустая строка.
— Никаких сервисных хвостов (время обработки, токены, режим).

Текст:
{{text}}

Вот такие параметры: 
Рекомендованные параметры: model: gpt-5-mini, temperature: 0.2–0.25, frequency_penalty: 0.35, reasoning: {{ effort: "minimal" }}, max_tokens: 450–600."""

...
class SummaryEngine:
    ...
    def _get_chat_system_prompt(self) -> str:
        ...
        return f"""Сожми текст/транскрипт ниже. Формат — максимально лаконичный и последовательный.
Опорная дата: {today_iso} (Europe/Amsterdam).

Формат:
▶ КОНСПЕКТ (по порядку, 3–5 строк): • …
📊 ДАННЫЕ/ЦИФРЫ: • тип — значение — контекст/цитата (только интересные)
⚡ ДЕЙСТВИЕ: одна строка ИЛИ «нет»
🎭 ТОН: 1–2 слова

Жёсткие запреты:
— Не повторяй один и тот же факт в разных блоках.
— Не пиши общие формулировки типа «обсуждение вида/планов».
— «Пиши/подумать/обсудить» без результата — не считать действием."""
